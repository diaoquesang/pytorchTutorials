...
class myTransformMethod2():  # Python3默认继承object类
    def __call__(self, img):  # __call___，让类实例变成一个可以被调用的对象，像函数
...
if __name__ == "__main__":
    ...
    model.eval()  # 设置推理模式
    myTransform = transforms.Compose([myTransformMethod1(), transforms.ToTensor(),myTransformMethod2()])
    ...
    for i in range(10):
        ...
        print(model(img),torch.argmax(model(img),dim=1))
